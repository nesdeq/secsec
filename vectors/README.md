...
- **`secsec-kat-v1.txt`** — hex vectors chained from one root key (`master_key = 0x11×32`, generation
  ...
  (§9.8 mutable head blob), `[auth]` (§11 session transcript), and `[roster]` (§9.5 per-entry AEAD +
  §8.2 roster-key history). Each value is the exact output of the reference Rust implementation.
