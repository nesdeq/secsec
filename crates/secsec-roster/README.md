...
The roster sigchain — entries, fold/succession, and the key-management layers wrapped around it
(`secsec-Design.md` §8). **This is the real ACL.**
...
bootstrap fold (§8.1), and the revoke⇒rotate op builder (§8.4). New devices are enrolled via
**invite-code pairing** (`secsec-client::pair`), which appends the `AddDevice` op + keyslot wrap over
the wire (§7). The §8.1 anti-rollback frontier (highest accepted seq + stored tip-blob hash) is
persisted and enforced by the client cold-start (`secsec-client::repo::open_repo_remote`).
