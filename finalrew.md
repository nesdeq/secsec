...
[MEDIUM] HistoryReanchor breaks sigchain folding. Roster entries are encrypted with the generation-indexed roster_key_g (the FA-2 fix). Folding the chain for succession requires decrypting every entry from genesis. But HistoryReanchor (§8.1/§8.2) drops key-history below drop_before_gen, so a freshly-enrolled device can no longer derive roster_key_g for the dropped generations → it cannot decrypt the old entries → it cannot verify succession from genesis. As written this is unsound in that corner. It only triggers at 256 rotations (rare for single-user), but the spec needs a signed membership-snapshot baseline carried in the reanchor entry so a fresh device can adopt the reanchored state as a new trusted root.
