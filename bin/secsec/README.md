...
link under `~/.config/secsec/folders/`). There are no local-store admin commands and no `prune`
command — history retention runs automatically inside `sync`. Run `secsec <command> --help` for flags.
